...
## build_front_linux: builds the frontend linux binary as a linux executable
build_front_linux:
	@echo Building frontend linux binary...
	chdir .\front-end && set GOOS=linux&& set GOARCH=amd64&& set CGO_ENABLED=0 && go build -o ${FRONT_BINARAY} ./cmd/web
	@echo Done!
